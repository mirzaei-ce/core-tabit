...
###   This script attempts to download the signature file SHA256SUMS.asc from tabit.org
...
WORKINGDIR="/tmp/tabit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://tabit.org/bin/"
VERSIONPREFIX="tabit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'tabit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Tabit Core binary release signing key installed?"
   fi
